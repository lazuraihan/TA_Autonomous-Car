...
void cloudCallback(const sensor_msgs::PointCloud2::ConstPtr& pt_cloud){
	ROS_INFO("PointCloud received: frame_id = %s, seq = %d", pt_cloud->header.frame_id.c_str(), pt_cloud->header.seq);
...
void showGridMap(unsigned int id) {
	...
	// Check if the grid_map_thresh is empty
    if (grid_map_thresh.empty()) {
        ROS_WARN("grid_map_thresh is empty. Aborting showGridMap.");
        return;
    }

    // Display the thresholded grid map
    cv::Mat display_image;
    cv::resize(grid_map_thresh, display_image, cv::Size(), resize_factor, resize_factor, cv::INTER_NEAREST);
    ROS_INFO("Resized grid_map_thresh to display_image with size (%d, %d)", display_image.rows, display_image.cols);

    std::string window_name = "Grid Map " + std::to_string(id);
    cv::imshow(window_name, display_image);
    ROS_INFO("Displaying grid map in window: %s", window_name.c_str());

    // Wait for a key press to close the window
    //int key = cv::waitKey(1);
    ROS_INFO("Waiting for key press to close the window. Key: %d", key);
	...
	if (key == 27) {
	    ...
	    ROS_INFO("showGridMap called with id: %u", id);
